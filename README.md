...
- Scheduled giveaways:
  - `scheduling.auto_enabled`: global toggle for daily automation at startup.
  - Each entry under `scheduling.giveaways` needs `id`, `channel_id`, `winners`, `title`, `description`, `start_time`, and `end_time` (24h clock).
...
- Permissions:
  - Provide role IDs in `permissions.admin_roles` to allow those roles to manage giveaways (otherwise users need the Discord `Manage Server` permission).

## Commands
- `/giveaway-start <channel> <winners> <title> <description>`: creates an active giveaway using the default manual duration.
...
- `/giveaway-logger <channel>`: set the channel for log messages (persists in `data/state.json`).
